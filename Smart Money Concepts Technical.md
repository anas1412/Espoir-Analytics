# Smart Money Concepts (SMC) - Full Technical Documentation & Logic

This document contains the complete logic, definitions, and mathematical implementations for the Smart Money Concepts (SMC) library. Use this as the definitive source of truth for implementing these features.

## 1. Input Validation & Base Configuration
All functions require a Pandas DataFrame containing OHLCV data. The data is normalized to lowercase column names before processing.
...
**Definition:** A fair value gap is when the previous high is lower than the next low if the current candle is bullish. Or when the previous low is higher than the next high if the current candle is bearish.
**Returns:** FVG direction (1/-1), Top level, Bottom level, and the Index where the gap was mitigated.
...
**Definition:** A swing high is when the current high is the highest high out of the `swing_length` amount of candles before and after. A swing low is the lowest low out of the same period.
**Returns:** HighLow direction (1/-1) and the price Level of the swing.
...
**Definition:** Indications of market structure changing. BOS continues trend, CHoCH shifts the trend.
**Returns:** BOS (1/-1), CHOCH (1/-1), the price Level, and the BrokenIndex (candle that broke the level).
...
## 5. Order Blocks (OB)
**Definition:** Detects order blocks when there is a high amount of market orders existing on a specific price range.
**Returns:** OB direction (1/-1), Top/Bottom, OBVolume, Percentage (strength), and MitigatedIndex.
...
## 6. Liquidity
**Definition:** Identifies pools of liquidity when multiple highs or lows occur within a small percentage range of each other.
**Returns:** Liquidity direction (1/-1), Level, End index, and Swept index.
...
## 7. Previous High / Low
**Definition:** Returns the previous high and low of a given higher time frame (e.g. 15m, 1H, 1D).
**Returns:** PreviousHigh, PreviousLow, BrokenHigh (1/0), BrokenLow (1/0).
...
## 8. Sessions
**Definition:** Determines which candles are within a specified trading session (e.g. London, New York).
**Returns:** Active (1/0), and the rolling High/Low of the session.
...
## 9. Retracements
**Definition:** Calculates the percentage of a retracement from the most recent swing high or low.
**Returns:** Direction (1/-1), CurrentRetracement%, and DeepestRetracement%.
...
## 10. Intermediate Term Highs & Lows (ITH / ITL)
**Definition:** 
*   **ITH (Intermediate Term High):** A swing high that forms inside a bearish Fair Value Gap (FVG).
*   **ITL (Intermediate Term Low):** A swing low that forms inside a bullish Fair Value Gap (FVG).

**Parameters:** Requires the standard `ohlc` DataFrame, along with the results from the `swing_highs_lows` and `fvg` functions.
**Returns:** ITH_ITL (1 for ITH, -1 for ITL, NaN otherwise).

```python
    @classmethod
    def ith_itl(cls, ohlc: DataFrame, swing_highs_lows: DataFrame, fvg_data: DataFrame) -> Series:
        """
        Identifies Intermediate Term Highs (ITH) and Intermediate Term Lows (ITL).
        ITH = Swing High inside a Bearish FVG.
        ITL = Swing Low inside a Bullish FVG.
        """
        ...
        fvg_val = fvg_data["FVG"].values
        fvg_top = fvg_data["Top"].values
        fvg_bottom = fvg_data["Bottom"].values
        fvg_mitigated = fvg_data["MitigatedIndex"].values
            ...
            if shl_val[i] == 1:  # Swing High
                ...
                for j in range(i):
                    if fvg_val[j] == -1:  # Bearish FVG
                        # Active if not mitigated yet, or mitigated AT or AFTER this swing high
                        is_active = np.isnan(fvg_mitigated[j]) or fvg_mitigated[j] >= i
                ...
                for j in range(i):
                    if fvg_val[j] == 1:  # Bullish FVG
                        # Active if not mitigated yet, or mitigated AT or AFTER this swing low
                        is_active = np.isnan(fvg_mitigated[j]) or fvg_mitigated[j] >= i
                        if is_active and (fvg_bottom[j] <= shl_level[i] <= fvg_top[j]):
        ...
        return pd.Series(ith_itl, name="ITH_ITL")
```
